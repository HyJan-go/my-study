...
/**
 * @program: study
 * @description: java操作es查询操作测试
 * 这个测试没有成功，推荐自己去使用springboot集成，会更简单。
 * @author: HyJan
...
public class ElastaicTest {
    ...
    public void before() throws Exception
    {
        ...
        Settings setting = Settings.builder()
                .put("cluster.name", "docker-cluster")//指定集群名称
        ...
        TransportAddress transportAddress = new TransportAddress(InetAddress.getByName("192.168.75.134"),9300);
...
//        }
    ...
    {
        Student stu = new Student("就是这么搞笑",25);
        stu.setName("Fresh");
        stu.setAge(22);
...
//    {
//        long count = transportClient.prepareGet(index).get().getCount();
...
//    }
    ...
    public void testSearchsAndTimeout()
    {
        SearchResponse searchResponse = transportClient.prepareSearch("shb01","shb02").setTypes("stu","tea")
                ...
                .setSearchType(SearchType.QUERY_THEN_FETCH)
                .setTimeout(new TimeValue(3, TimeUnit.SECONDS))
                .get();
...
//                .setPostFilter(FiltersAggregationBuilder.parse("age").gte(18).lte(22))
